...
import es.caib.emiserv.core.service.RedireccioServiceImpl;
import es.caib.emiserv.core.service.ws.DatosEspecificosHandler;
import es.caib.emiserv.core.service.ws.EmiservBackofficeImpl;
...
public class BackofficeHelper {
	...
	public RespuestaAmbException solicitudResposta(
		...
		ScspCorePeticionRespuestaEntity scspPeticionRespuesta = scspCorePeticionRespuestaRepository.findOne(idPeticion);
	...
	public void processarPeticioPendent(
			Long peticioId) {
		BackofficePeticioEntity peticio = backofficePeticioRepository.findOne(peticioId);
	...
	public ScspCoreServicioEntity findCoreServicioPerPeticionRespuesta(
		...
		ScspCoreServicioEntity scspCoreServicio = scspCoreServicioRepository.findOne(peticionRespuesta.getCertificado());
	...
	private void peticioBackofficeEnviarNomesUna(
		...
		ScspCoreServicioEntity scspCoreServicio = scspCoreServicioRepository.findOne(scspPeticionRespuesta.getCertificado());
		if (scspCoreServicio == null) {
			excepcio = new NotFoundException(
	...
	private EmiservBackoffice getEmiservBackoffice(
			...
			DatosEspecificosHandler datosEspecificosHandler) throws InstanceNotFoundException, MalformedObjectNameException, RemoteException, NamingException, CreateException, MalformedURLException {
	...
	private boolean getBackofficeProcessarDatosEspecificosPeticio(String serveiCodi) {
		String processar = PropertiesHelper.getProperties().getProperty(
				"es.caib.emiserv.backoffice." + serveiCodi + ".processar.datos.especificos.peticio");
		if (processar == null) {
			processar = PropertiesHelper.getProperties().getProperty(
	...
	private class EmiservBackofficeHandlerProxy implements EmiservBackoffice {
		...
		//private DatosEspecificosHandler datosEspecificosHandler;
		...
		/*public void setDatosEspecificosHandler(DatosEspecificosHandler datosEspecificosHandler) {
			this.datosEspecificosHandler = datosEspecificosHandler;
		}*/
		private String toXml(
				Object obj) {
			try {
				/*new JAXBElement<Peticion>(
						_BillingAddress_QNAME,
						AddressType.class,
						null,
						value);
				Class<?> clazz;
				if (obj instanceof Peticion) {
					clazz = es.scsp.bean.common.Peticion.class;
	...
	private boolean isPropertyBackofficeMock() {
		return PropertiesHelper.getProperties().getAsBoolean("es.caib.emiserv.backoffice.mock");
	}
